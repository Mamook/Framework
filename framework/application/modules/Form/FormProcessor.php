...
class FormProcessor
{
            ...
            if (array_key_exists('_submit_check', $_POST)) {
                ...
                foreach ($all_branches as $row) {
                    ...
                    if (isset($_POST[$branch_id]) && $_POST[$branch_id] == 'on') {
                        ...
                        $user_message = '<ul><li>' . implode('<li></li>',
                                $message) . '</li></ul>' . '<strong>The manager(s) have been notified. You will be sent an email notifying you of the results.</strong>';
     ...
     *
     * @param array $branch_ids Each value must be the POST Data index that exactly matches the name of the field in
        ...
        if (array_key_exists('_submit_check', $_POST)) {
            try {
                ...
                try {
                    ...
                    $user_obj->updateUser(array('ID' => $id),
                        array('newsletter' => $newsletter, 'notify' => $notify_ids, 'questions' => $questions));
                    ...
                    throw new Exception('There was an error updating the privacy settings for user ID: ' . $id . ' in the Database: ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                        E_RECOVERABLE_ERROR);
                }
    ...
    public function processUsername($id = null)
    {
        ...
        if (array_key_exists('_submit_check', $_POST)) {
            ...
            $empty_username = $validate->validateEmpty('username',
                'Please enter a username that is at least 5 characters long.', 5, 64);
            ...
            $empty_username_conf = $validate->validateEmpty('confirmed_username', 'Please confirm your new username.',
                5, 64);
            ...
            if ($validate->checkErrors() === true) {
                ...
                if (WP_INSTALLED === true) {
                    ...
                    try {
                        ...
                        throw new Exception('There was an error updating the username for WordPress user ID: ' . $wp_id . ' in the WordPress Database: ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                            E_RECOVERABLE_ERROR);
                    }
                }
                try {
                    ...
                    throw new Exception('There was an error updating the username for user ID: ' . $id . ' in the Database: ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                        E_RECOVERABLE_ERROR);
                }
    ...
    protected function processReset($submit_button = 'post', $session_index = null)
    {
        try {
            ...
            if (array_key_exists('_submit_check',
                    $_POST) && isset($_POST[$submit_button]) && ($_POST[$submit_button] === 'Reset')) {
