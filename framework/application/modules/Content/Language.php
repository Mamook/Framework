    ...
    public function displayLanguageList($select = false)
    {
        ...
        try {
            ...
            if ($content_count > 0) {
                ...
                $and_sql = '';
                # Get the Languages.
                $this->getLanguages($paginator->getRecordOffset() . ', ' . $paginator->getRecordsPerPage(), '*',
                ...
                foreach ($all_languages as $row) {
                    ...
                    $language_name = str_ireplace(array('%{domain_name}', '%{site_name}', '%{fw_popup_handle}'),
                        array(DOMAIN_NAME, $site_name, FW_POPUP_HANDLE), $language->getLanguage());
                    ...
                    if ($select === 'select') {
                        ...
                        $fg->addElement('radio', array(
                            'name' => 'language_info',
                            'value' => $language_id . ':' . $language_name,
                ...
                if ($select === 'select') {
                    ...
                    $fg->addElement('submit', array('name' => 'language', 'value' => 'Select'), '', null,
                        'submit-language');
                    # Close the fieldset.
                    $fg->addFormPart('</fieldset>');
