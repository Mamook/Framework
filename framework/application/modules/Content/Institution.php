...
class Institution
{
    ...
    public function displayInstitutionList($select = false)
    {
        ...
        try {
            ...
            if ($content_count > 0) {
                ...
                $this->getInstitutions($paginator->getRecordOffset() . ', ' . $paginator->getRecordsPerPage(), '*',
                ...
                foreach ($all_institutions as $row) {
                    ...
                    $institution_name = str_ireplace(array('%{domain_name}', '%{site_name}', '%{fw_popup_handle}'),
                        array(DOMAIN_NAME, $site_name, FW_POPUP_HANDLE), $institution->getInstitution());
                    ...
                    if ($select === 'select') {
                        ...
                        $fg->addElement('radio', array(
                            ...
                            'value' => $institution_id . ':' . $institution_name,
                ...
                if ($select === 'select') {
                    ...
                    $fg->addElement('submit', array('name' => 'institution', 'value' => 'Select'), '', null,
                        'submit-institution');
                    # Close the fieldset.
                    $fg->addFormPart('</fieldset>');
     ...
     * and sets it to the data member.
     *
