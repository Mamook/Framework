        ...
        if ($publisher === true) {
            ...
            if ($cont_id !== null) {
                ...
                if ($cont_privacy !== null) {
                    ...
                    $profile_contributor .= '<span class="label">Posted by</span> <a href="' . APPLICATION_URL . 'profile/?contributor=' . $cont_id . '" title="' . $contributor->getContName() . '">' . $contributor->getContName() . '</a>' . (($date !== '0000-00-00') ? ' on <span class="post-date"><span class="post-month">' . date("F",
                                $date_timestamp) . '</span> <span class="post-day">' . date("d",
                                $date_timestamp) . '</span>, <span class="post-year">' . date("Y",
                                $date_timestamp) . '</span></span>' : '');
                ...
                if ($recent_cont_id !== null) {
                    ...
                    if ($recent_cont_privacy !== null) {
                        ...
                        $profile_recent_contributor .= '<span class="label">Edited by</span> <a href="' . APPLICATION_URL . 'profile/?contributor=' . $recent_cont_id . '" title="' . $recent_contributor->getContName() . '">' . $recent_contributor->getContName() . '</a> on <span class="edit-date"><span class="edit-month">' . date("F",
    ...
    public function displayPublisherList($select = false)
    {
        ...
        try {
            ...
            if ($content_count > 0) {
                ...
                $and_sql = '';
                # Get the Publishers.
                $this->getPublishers($paginator->getRecordOffset() . ', ' . $paginator->getRecordsPerPage(), '*',
                    $sort_by, $sort_dir, $and_sql);
                ...
                if ($select === 'select') {
                    ...
                    $fg = new FormGenerator('post',
                        'http' . ($validator->isSSL() === true ? 's' : '') . '://' . FULL_URL, 'post', '_top', false,
                        'publisher-list');
                ...
                foreach ($all_publishers as $row) {
                    ...
                    # Replace any tokens with their correlating value.
                    $publisher_info = str_ireplace(array('%{domain_name}', '%{site_name}', '%{fw_popup_handle}'),
                        array(DOMAIN_NAME, $site_name, FW_POPUP_HANDLE), $publisher->getInfo());
                    $publisher_id = $publisher->getID();
                    # Replace any tokens with their correlating value.
                    $publisher_name = str_ireplace(array('%{domain_name}', '%{site_name}', '%{fw_popup_handle}'),
                        array(DOMAIN_NAME, $site_name, FW_POPUP_HANDLE), $publisher->getPublisher());
                ...
                if ($select === 'select') {
                    ...
                    $fg->addElement('submit', array('name' => 'publisher', 'value' => 'Select'), '', null,
                        'submit-publisher');
                    # Close the fieldset.
                    $fg->addFormPart('</fieldset>');
