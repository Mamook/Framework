...
class Content
{
     ...
     *
     * @param array|string $all_content May be an array or a string. The method makes it into an array regardless.
     */
    protected function setAllContent($all_content)
    {
        ...
        if (!empty($all_content)) {
            # Set the data member.
            $this->all_content = (array)$all_content;
        } else {
    ...
    public function setPageTitle($page_title, $form = null)
    {
        ...
        if (!empty($page_title)) {
            ...
            $page_title = str_ireplace(array('%{domain_name}', '%{site_name}', '%{fw_popup_handle}'),
                array(DOMAIN_NAME, $site_name, FW_POPUP_HANDLE), $page_title);
        } else {
    ...
    public function setSubTitle($sub_title)
    {
        ...
        if (!empty($sub_title)) {
            ...
            # Replace any tokens with their correlating value.
            $this->sub_title = str_ireplace(array('%{domain_name}', '%{site_name}', '%{fw_popup_handle}'),
                array(DOMAIN_NAME, $site_name, FW_POPUP_HANDLE), $sub_title);
        } else {
            # Explicitly set the data member to NULL.
            $this->sub_title = null;
        }
    ...
    public function displayContentList()
    {
        ...
        try {
            ...
            if ($content_count > 0) {
                ...
                # Get the Files.
                $this->getContentPages($paginator->getRecordOffset() . ', ' . $paginator->getRecordsPerPage(), '*',
    ...
    public function displayImage($image_link = FW_POPUP_HANDLE)
    {
        ...
        if (!empty($image_name)) {
            ...
            $content .= $image->displayImage(true, $image_name,
                ((empty($image_title)) ? strip_tags($this->getPageTitle()) : $image_title), $image_link);
        }
    ...
    /**
     * Retrieves content info from the `content` table in the Database for the passed id or page name and sets it to
     * the data member.
     *
