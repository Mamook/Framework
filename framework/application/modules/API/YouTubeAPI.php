...
class YouTube
{
    ...
    public function getYouTubeIdFromEmbedCode($embed_code)
    {
        ...
        if (!empty($embed_code)) {
            ...
            if ($embed_code == strip_tags($embed_code)) {
                ...
                preg_match('/\b(?:(?:https?):\/\/|www\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i', $embed_code,
                    $matches);
    ...
    /**
     * insertThumbnail
     * Create a request for the API's thumbnails.set method to upload the image and associate it with the appropriate
     * video.
     *
     ...
     *                                            snippet, contentDetails, player, statistics, status, and
     *                                            topicDetails. However, not all of those parts contain properties that
     *                                            can be set when setting or updating a video's metadata. For example,
     *                                            the statistics object encapsulates statistics that YouTube calculates
     *                                            for a video and does not contain values that you can set or modify.
     *                                            If the parameter value specifies a part that does not contain mutable
     ...
     * @param    array  $optParams                Optional parameters.
     *                                            string hl                    The hl parameter specifies the language
     *                                            that should be used for text values in the API response. string id
     *                                                            The id parameter specifies a comma-separated list of
     *                                                            video category IDs for the resources that you are
     *                                                            retrieving. string regionCode            The
     *                                                            regionCode parameter instructs the API to return the
     ...
     * @return array
     */
    public function PlaylistsListFeed(
        $part = 'snippet',
        $optParams = array('channelId' => YOUTUBE_CHANNELID, 'maxResults' => 50)
    ) {
        # Bring $youtube_service into the scope.
        $youtube_service = $this->getYouTubeService();
     ...
     * @param array     $playlistsItems_optParams
     *
     * @return array
     */
    public function PlaylistItems(
        $playlist_id,
        $part = 'snippet',
        $playlistsItems_optParams = array('maxResults' => 50)
    ) {
        # Bring $youtube_service into the scope.
        $youtube_service = $this->getYouTubeService();
    ...
    public function searchYouTube(
        ...
        $part = 'snippet',
        $order = 'date',
        $search_optParams = array('maxResults' => 50)
    ) {
        # Bring $youtube_service into the scope.
        $youtube_service = $this->getYouTubeService();
    ...
    private function rebuildArray(
        $array,
        $is_playlist_items = false,
        $is_video_details = false,
        $is_playlist_feed = false
    ) {
        # Create a new array
        $new_array = array();
