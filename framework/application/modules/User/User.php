...
class User
{
    ...
    public function checkLogin($levels)
    {
        ...
        if (($access === false) && (strpos(Utility::removeIndex(LOGIN_PAGE),
                    Utility::removeIndex(FULL_URL)) === false)) {
        ...
        elseif (($access === true) && (strpos(Utility::removeIndex(LOGIN_PAGE),
                    Utility::removeIndex(FULL_URL)) !== false)) {
            $doc->redirect(REDIRECT_AFTER_LOGIN);
        }
    }
                ...
                if ($row === null) {
                    ...
                    if ((int)$this->getNewsletter() === 1) {
                        try {
                            ...
                            throw new Exception('There was an error confirming ' . $username . '\'s newsletter subscription in the Database: ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                                E_RECOVERABLE_ERROR);
                        } catch (Exception $e) {
                            throw $e;
                        }
    ...
    public function unsubscribeNewsletter($user_id, $redirect = false)
    {
        try {
            ...
            throw new Exception('There was an error unsubscribing user ' . $user_id . ': ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    public function createAccount()
    {
        try {
            ...
            $insert_user = $db->query('INSERT INTO `' . DBPREFIX . 'users` (`display`, `username`, `email`, `password`, `random`, `registered`, `reg_ip`) VALUES (' . $db->quote($db->escape($username)) . ', ' . $db->quote($db->escape($username)) . ', ' . $db->quote($db->escape($email)) . ', ' . $db->quote($db->escape($password)) . ', ' . $db->quote($db->escape($this->randomString('alnum',
                    32))) . ', ' . $db->quote($db->escape(YEAR_MM_DD)) . ', ' . $db->quote($db->escape($ip)) . ')');
            ...
            throw new Exception('There was an error registering a new user: ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    public function createWP_User()
    {
        ...
        try {
            ...
            throw new Exception('There was an error inserting the new WordPress user info for "' . $username . '" into the Database: ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        }
        ...
        try {
            ...
            throw new Exception('There was an error inserting the new WordPress usermeta info for "' . $username . '" into the Database: ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        }
    }
    ...
    public function deleteAccount($id = null)
    {
        ...
        try {
			...
			*/
            ...
            throw new Exception('There was an error deleting User ID# ' . $id . ' from the Database: ' . $ez->error . '<br />Code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
    ...
    public function findEmail($id = null)
    {
        ...
        try {
            ...
            throw new Exception('There was an error: ' . $ez->error . '<br />Code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    public function findPassword($user = null, $field = null)
    {
        ...
        try {
            ...
            throw new Exception('There was an error finding the user\'s password: ' . $ez->error . '<br />Code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    public function findProduct($field = null)
    {
        ...
        try {
            ...
            try {
                ...
                throw new Exception('There was an error retrieving the user\'s product: ' . $ez->error . '<br />Code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                    E_RECOVERABLE_ERROR);
            } catch (Exception $e) {
                throw $e;
            }
    ...
    public function findStaffID($value = null)
    {
        ...
        try {
            ...
            throw new Exception('There was an error retrieveing the user\'s staff ID: ' . $e->error . ', code: ' . $e->errno . '<br />Last query: ' . $e->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    public function findUserData($value = null)
    {
        ...
        try {
            ...
            throw new Exception('There was an error retrieveing the user\'s data: ' . $e->error . ', code: ' . $e->errno . '<br />Last query: ' . $e->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    public function findUserID($username = null)
    {
        ...
        try {
            ...
            throw new Exception('There was a Database error: ' . $ez->error . '<br />Code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    public function findUserLevel($field = null)
    {
        ...
        try {
            ...
            try {
                ...
                throw new Exception('There was an error checking the user\'s access level: ' . $ez->error . '<br />Code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                    E_RECOVERABLE_ERROR);
            } catch (Exception $e) {
                throw $e;
            }
    ...
    public function findUsername($field = null)
    {
        ...
        try {
            ...
            throw new Exception('There was an error finding the user\'s username: ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    public function getPrivacySettings($username)
    {
        ...
        try {
            ...
            $row = $db->get_row('SELECT `newsletter`, `notify`, `questions` FROM `' . DBPREFIX . 'users` WHERE `username` = ' . $db->quote($db->escape(htmlspecialchars($username,
                    ENT_QUOTES, 'UTF-8', false))) . ' LIMIT 1');
            if ($row !== null) {
                # Set the returned data to the data members.
                $this->setNewsletter($row->newsletter);
                $this->setNotify($row->notify);
            ...
            throw new Exception('There was an error retrieveing the user\'s privacy settings: ' . $e->error . '<br />Code: ' . $e->errno . '<br />Last query: ' . $e->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    public function getSubscriptions($user_id = null)
    {
        ...
        $subscription->getSubscriptions(null, '`id`, `name`, `date`', 'id', 'ASC',
            ' WHERE `user` = ' . $db->quote($db->escape($user_id)));
            ...
            if (isset($_COOKIE['cookie_id'])) {
                ...
                if (($_COOKIE['authenticate'] == $authenticate) && (md5($ip) == $_COOKIE['ip'])) {
                    ...
                    try {
                        ...
                        $this->setLoginSessions($id, $display, $password, $fname, $lname, $title, $registered,
                            $last_login, true, true);
    ...
    public function sendAccountInfo($email)
    {
        try {
            ...
            throw new Exception('There was an error retrieving the "random" field for the user with the email address"' . $email . '" from the Database: ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        }
    }
    ...
    public function sendActivationEmail($email, $new_account = false)
    {
        try {
            ...
            throw new Exception('There was an error retrieving the new user info for "' . $email . '" from the Database: ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        }
    }
     ...
     * @throws Exception
     */
    public function setLoginSessions(
        $user_id = null,
        $display_name = null,
        $password = null,
        $fname = null,
        ...
        if (isset($_COOKIE[SESSIONS_NAME])) {
            setcookie(SESSIONS_NAME, $_COOKIE[SESSIONS_NAME], (($remember !== true) ? LOGIN_LIFE_SHORT : LOGIN_LIFE),
                COOKIE_PATH, '.' . DOMAIN_NAME, $secure);
        }
    ...
    public function updateUser($where_field, $field_value)
    {
        ...
        try {
            ...
            throw new Exception('There was an error updating User info: ' . $ez->error . '<br />Code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    public function getInactiveUsers($user_id = null)
    {
        ...
        try {
            if ($user_id !== null) {
                ...
                $results = $db->get_results('SELECT `user_id` FROM `' . DBPREFIX . 'users_inactive` WHERE `delete_date` <= CURDATE()',
                    ARRAY_N);
            }

            return $results;
        } catch (ezDB_Error $e) {
            throw new Exception('There was an error checking the users_inactive table: ' . $e->error . ', code: ' . $e->errno . '<br />Last query: ' . $e->last_query,
                E_RECOVERABLE_ERROR);
        }
    }
    ...
    public function deleteInactiveUsers()
    {
        try {
            ...
            throw new Exception('There was an error deleting the inactive user: ' . $e->error . ', code: ' . $e->errno . '<br />Last query: ' . $e->last_query,
                E_RECOVERABLE_ERROR);
        }
    }
    ...
    protected function deleteInactiveUser($user_id)
    {
        ...
        try {
            ...
            throw new Exception('There was an error deleting the user from the user_inactive table: ' . $e->error . ', code: ' . $e->errno . '<br />Last query: ' . $e->last_query,
                E_RECOVERABLE_ERROR);
        }
    }
