...
class Staff
{
    ...
    public function setText($text)
    {
        if (!empty($text)) {
            ...
            $text = str_ireplace(array('%{domain_name}', '%{fw_popup_handle}'), array(DOMAIN_NAME, FW_POPUP_HANDLE),
                $text);
            # Set the data member.
            $this->text = $text;
        } else {
     ...
     *
     * @param    string  $value May be the staff id, the person's User ID, the person's email, or the person's first
     *                          and last names. Names must be in an array.
    ...
    public function getStaffName($value = null)
    {
        ...
        try {
            ...
            throw new Exception('There was an error finding the staff\'s name: ' . $ez->error . ', code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        } catch (Exception $e) {
            throw $e;
        }
    ...
    /**
     * getThisStaff
     * Retrieves a staff's info from the `staff` table in the database for the passed value and related field and sets
     * it to the data member.
     *
    ...
    public function updateStaff($where_field, $field_value)
    {
        ...
        try {
            ...
            throw new Exception('There was an error updating staff info: ' . $ez->error . '<br />Code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                E_RECOVERABLE_ERROR);
        }
    }
