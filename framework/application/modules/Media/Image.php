...
class Image extends Media
{
                ...
                if ($validator->isInt($id) === true) {
                    ...
                    if (($file_handler->deleteFile(IMAGES_PATH . $image_name) === true) && ($file_handler->deleteFile(IMAGES_PATH . 'original' . DS . $image_name) === true)) {
                        try {
                            ...
                            throw new Exception('Error occured: ' . $ez->error . ', but the image itself was deleted.<br />Code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                                E_RECOVERABLE_ERROR);
                        } catch (Exception $e) {
                            throw $e;
                        }
     ...
     *
     * @param bool   $return      TRUE to return the string, FALSE to echo it.
     * @param string $image_name  The name of the image to display.
     * @param string $image_title The title of the image to display.
    ...
    public function displayImage(
        ...
        try {
            ...
            if (!empty($image_name)) {
                ...
                $display_image .= str_replace('%{insert_content}',
                    '<img src="' . IMAGES . $image_name . '" class="image" alt="' . $this->getTitle() . '"/>',
                    $image_link);
            }
            if ($return === false) {
                echo $display_image;
    ...
    public function displayImageList($select = false)
    {
        ...
        try {
            ...
            if ($content_count > 0) {
                ...
                # Get the Images.
                $this->getImages($paginator->getRecordOffset() . ', ' . $paginator->getRecordsPerPage(), '*', $sort_by,
                    $sort_dir);
                ...
                foreach ($all_images as $row) {
                    ...
                    $image_desc = str_ireplace(array('%{domain_name}', '%{fw_popup_handle}'),
                        array(DOMAIN_NAME, FW_POPUP_HANDLE), $image_obj->getDescription());
                    $image_id = $image_obj->getID();
                    $image_name = str_ireplace(array('%{domain_name}', '%{fw_popup_handle}'),
                        array(DOMAIN_NAME, FW_POPUP_HANDLE), $image_obj->getImage());
                    $image_title = str_ireplace(array('%{domain_name}', '%{fw_popup_handle}'),
                        array(DOMAIN_NAME, FW_POPUP_HANDLE), $image_obj->getTitle());
                ...
                if ($select === 'select') {
                    ...
                    $fg->addElement('submit', array('name' => 'image', 'value' => 'Select Image'), '', null,
                        'submit-image');
    ...
    public function getThisImage($value, $id = true)
    {
        ...
        try {
            ...
            if ($image !== null) {
                ...
                $description = str_ireplace(array('%{domain_name}', '%{fw_popup_handle}'),
                    array(DOMAIN_NAME, FW_POPUP_HANDLE), $description);
                # Strip slashes and decode any html entities.
                $description = ((empty($description)) ? '' : html_entity_decode(stripslashes($description), ENT_COMPAT,
                    'UTF-8'));
