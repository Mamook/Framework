...
class Media
{
                        ...
                        if (strpos($category, 'YouTube') !== false) {
                            ...
                            if ($validator->isInt($category_id)) {
                                ...
                                $db->query('INSERT INTO `' . DBPREFIX . 'categories` (' .
                                    ...
                                    '`api`' .
                                    ') VALUES (' .
                                    $db->quote($db->escape(str_ireplace(array(DOMAIN_NAME), array('%{domain_name}'),
                                        $name))) . ', ' .
                                    $db->quote($api) .
                                    ')'
                                );
    ...
    protected function setContent($content)
    {
        ...
        if (!empty($content)) {
            ...
            $content = str_ireplace(array('%{domain_name}', '%{fw_popup_handle}'), array(DOMAIN_NAME, FW_POPUP_HANDLE),
                $content);
            # Set the data member.
    ...
    protected function setLink($link)
    {
        ...
        if (!empty($link)) {
            ...
            $link = trim($link);
            # Replace any tokens with their correlating value.
            $link = str_ireplace(array('%{domain_name}', '%{fw_popup_handle}'), array(DOMAIN_NAME, FW_POPUP_HANDLE),
                $link);
            # Set the data member.
            $this->link = $link;
        } else {
    ...
    protected function setPurchaseLink($purchase_link)
    {
        ...
        if (!empty($purchase_link)) {
            ...
            # Replace any tokens with their correlating value.
            $purchase_link = str_ireplace(array('%{domain_name}', '%{fw_popup_handle}'),
                array(DOMAIN_NAME, FW_POPUP_HANDLE), $purchase_link);
            # Set the data member.
