                    ...
                    if ($this_file === true) {
                        ...
                        if ($file_handler->deleteFile(BODEGA . $folder . DS . $this->getFile()) === true) {
                            try {
                                ...
                                throw new Exception('Error occured: ' . $ez->error . ', but the file itself was deleted.<br />Code: ' . $ez->errno . '<br />Last query: ' . $ez->last_query,
                                    E_RECOVERABLE_ERROR);
                            } catch (Exception $e) {
                                throw $e;
                            }
        ...
        try {
            ...
            if ($content_count > 0) {
                ...
                */
                ...
                # Get the Files.
                $this->getFiles($paginator->getRecordOffset() . ', ' . $paginator->getRecordsPerPage(), '*', $sort_by,
                    $sort_dir);
                ...
                foreach ($all_files as $row) {
                    ...
                    $file_name = str_ireplace(array('%{domain_name}', '%{fw_popup_handle}'),
                        array(DOMAIN_NAME, FW_POPUP_HANDLE), $file_obj->getFile());
                    $file_title = str_ireplace(array('%{domain_name}', '%{fw_popup_handle}'),
                        array(DOMAIN_NAME, FW_POPUP_HANDLE), $file_obj->getTitle());
                ...
                if ($select === 'select') {
                    ...
                    $fg->addElement('submit', array('name' => 'file', 'value' => 'Select File'), '', null,
                        'submit-file');
